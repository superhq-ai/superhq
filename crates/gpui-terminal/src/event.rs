...
pub enum TerminalEvent {
    ...
    /// Contains a formatter that wraps the clipboard text in the OSC 52 response.
    ...
    fn send_event(&self, event: Event) {
        match event {
            ...
            Event::ClipboardLoad(_clipboard_type, format) => {
                self.send(TerminalEvent::ClipboardLoad(format));
            }
            Event::Exit => {
            ...
            Event::PtyWrite(data) => {
                // Terminal needs to write response data back to the PTY
                // (e.g., cursor position report, device attributes response).
...
mod tests {
    ...
    fn test_clipboard_load_event() {
        ...
        let event = rx.recv().unwrap();
        assert!(matches!(event, TerminalEvent::ClipboardLoad(_)));
    }
